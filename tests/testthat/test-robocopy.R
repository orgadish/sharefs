...
test_that("robocopy() requires source and destination to each be a single path, without checking robocopy first", {
  checked <- FALSE
  ...
  with_mocked_bindings(
    ...
    code = {
      expect_error(
        robocopy(c("a", "b"), withr::local_tempdir()),
        class = "sharefs_error_robocopy_bad_args"
      )
      expect_error(
        robocopy(withr::local_tempdir(), c("a", "b")),
        class = "sharefs_error_robocopy_bad_args"
      )
    }
  )
...
test_that("robocopy() treats files = character(0) as nothing to copy, without invoking robocopy", {
  ...
  with_mocked_bindings(
    ...
    code = {
      with_mocked_bindings(
        ...
        .package = "processx",
        code = {
          result <- robocopy(withr::local_tempdir(), withr::local_tempdir(), files = character(0))
        }
      )
    }
  )
...
test_that("robocopy() omits /XF and /XD entirely for exclude_files/exclude_dirs = character(0)", {
...
test_that("robocopy() propagates a processx-level failure (e.g. spawn/timeout) distinctly", {
  with_mocked_bindings(
    find_robocopy = function() c(robocopy = "/fake/robocopy"),
    code = {
      with_mocked_bindings(
...
test_that("robocopy() surfaces a spawn-failure message containing literal braces without erroring", {
...
test_that("robocopy() passes timeout through to processx::run(), defaulting to Inf", {
  ...
  with_mocked_bindings(
    ...
    code = {
      with_mocked_bindings(
        ...
        .package = "processx",
        code = robocopy(withr::local_tempdir(), withr::local_tempdir())
      )
    }
  )
  ...
  with_mocked_bindings(
    ...
    code = {
      with_mocked_bindings(
        ...
        .package = "processx",
        code = robocopy(withr::local_tempdir(), withr::local_tempdir(), timeout = 300)
      )
    }
  )
  ...
  with_mocked_bindings(
    ...
    code = {
      with_mocked_bindings(
        ...
        .package = "processx",
        code = robocopy(withr::local_tempdir(), withr::local_tempdir(), recurse = TRUE)
      )
    }
  )
  ...
  with_mocked_bindings(
    ...
    code = {
      with_mocked_bindings(
        ...
        .package = "processx",
        code = robocopy(withr::local_tempdir(), withr::local_tempdir(), move = TRUE)
      )
    }
  )
  ...
  with_mocked_bindings(
    ...
    code = {
      with_mocked_bindings(
        ...
        .package = "processx",
        code = robocopy(withr::local_tempdir(), withr::local_tempdir(), dry_run = TRUE)
      )
    }
  )
...
test_that("robocopy() omits /NFL /NDL /NJH /NJS when log_file is set, but always keeps /NP", {
  captured <- NULL
...
test_that("robocopy() handles source/destination paths containing spaces on a real Windows machine", {
...
test_that("robocopy() with log_file really writes a log with actual content", {
  skip_if_not(sfs_robocopy_available())
...
test_that("robocopy() can actually run a real, slow-ish operation without a timeout by default", {
