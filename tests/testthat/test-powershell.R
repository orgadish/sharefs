...
test_that("sfs_powershell_available() reflects find_powershell()", {
   local_mocked_bindings(find_powershell = function() {
      c(powershell = "/fake/path")
   })
...
test_that("run_powershell() prepends a UTF-8 output-encoding preamble", {
   ...
   # redirected, silently mangling non-ASCII output otherwise (confirmed
   # against a real run: accented/CJK/Cyrillic/emoji file names came back
   # replaced with '?' or the wrong character). This must run before any
   # of the caller's own script lines produce output.
   local_mocked_bindings(find_powershell = function() {
      c(powershell = "/fake/powershell")
   })
   ...
   # The preamble comes before the caller's own script.
   expect_true(
      which(grepl(
         "OutputEncoding",
         strsplit(script_arg, "\n")[[1]],
